...
def register_yannt_transformers(subparsers):
    transformers_parser = subparsers.add_parser("transformers", help="transformers command")
    # transformers_parser.add_argument("--breakpoint",
    #     dest="breakpoint",
    #     action="store_true",
    #     help="breakpoint() after operation"
    # )
    transformers_subparser = transformers_parser.add_subparsers(dest="transformers_command", required=True)

    transformers_list_parser = transformers_subparser.add_parser("list", help="transformers list command")
    transformers_list_parser.set_defaults(func=transformers_list)

    transformers_create_parser = transformers_subparser.add_parser("create", help="transformers create command")
...
def transformers_create(args):
    ...
    if len(args.pytorch_state_path) > 0:
        ...
        tmodel.save_torch_state(args.pytorch_state_path, f'{args.model_name}-{args.model_type}')
